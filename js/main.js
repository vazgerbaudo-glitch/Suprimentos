    ...
    for (let r = 0; r < 3; r++) {
        ...
        if (cBtn != null) {
            const mkBtn = (col, cls, label, aria, url) => {
                ...
                btn.className = cls;
                btn.style.left = col * stepX - shift + 'px';
                btn.style.width = WW + 'px';
                btn.style.height = H + 'px';
                btn.style.setProperty('--cf', CF + 'px');
...
function departSplash(el, btnRow, rows, url) {
    ...
    setTimeout(() => {
        ...
        else setTimeout(() => el.remove(), 400);
    }, 1100);
}

function onSplashResize() {
    const s = document.getElementById('splash');
    if (s && !s.classList.contains('depart')) buildSplash();
}

buildSplash();
window.addEventListener('resize', onSplashResize);

function applyRecords(recs, srcLabel) {
    ALL = recs;
    setSrc('live', srcLabel + ' · ' + ALL.length + ' registros · ' + new Date().toLocaleString('pt-BR'));
    buildFilters();
    render();
}
...
function loadEmpty(msg) {
    ALL = fromEmbedded();
    setSrc(msg ? 'err' : '', msg || 'Nenhuma base carregada');
    buildFilters();
    render();
}
...
function loadLocalBase(prefix) {
    fetch(BASE_CSV_PATH)
        ...
        .then(txt => {
            ...
            applyRecords(recs, 'Base local (' + BASE_CSV_PATH + ')');
        })
        .catch(() => { loadEmpty(prefix ? prefix + ' Exibindo base inicial.' : null); });
}
...
function loadSheet(url) {
    ...
    fetch(url)
        ...
        .then(txt => {
            const recs = fromCSV(txt).filter(r => r.dc || r.dl || r.st === 'A');
            if (!recs.length) throw new Error('vazio');
            applyRecords(recs, 'Conectado ao Google Sheets');
        })
        .catch(e => { loadLocalBase('Falha ao ler o Sheets (' + e.message + ').'); });
}

function loadData(url) {
    if (url) return loadSheet(url);
    loadLocalBase();
}
...
function loadCSVText(txt, nome) {
    try {
        ...
        setSrc('err', 'Falha ao ler o CSV (' + err.message + '). Exibindo base inicial. Confira o cabeçalho das colunas.');
    }
}
