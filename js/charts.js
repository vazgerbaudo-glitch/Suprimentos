...
// ---- helpers de cor ----
function colA(col, a) {
    if (typeof col !== 'string') return `rgba(70,96,111,${a})`;
...
function colLight(col) {
    const m = /^#([0-9a-f]{6})$/i.exec(col || '');
    if (!m) return col;
    const n = parseInt(m[1], 16), mx = c => Math.round(c + (255 - c) * .45);
    return `rgb(${mx(n >> 16 & 255)},${mx(n >> 8 & 255)},${mx(n & 255)})`;
}
...
const valLabels = {
    ...
    afterDatasetsDraw(chart) {
        ...
        chart.data.datasets.forEach((ds, di) => {
            ...
            if (meta.hidden || meta.type !== 'bar' || ds.stack) return;
            meta.data.forEach((el, i) => {
                const v = ds.data[i];
                if (v == null || typeof v === 'object' || isNaN(v) || !+v) return;
                if (el.width < 14) return;
                const c = Array.isArray(ds.backgroundColor) ? ds.backgroundColor[i] : ds.backgroundColor;
                ctx.fillStyle = colA(c, .6);
                const n = +v;
                ctx.fillText(Math.abs(n) >= 1e4 ? Kf(n) : n.toLocaleString('pt-BR', { maximumFractionDigits: 2 }), el.x, el.y - 3);
            });
        });
        ctx.restore();
    }
...
const hoverPills = {
    ...
    afterEvent(chart, args) {
        if (chart.options.indexAxis !== 'y') return;
        const on = args.event.type !== 'mouseout';
        if (chart.$pills !== on) { chart.$pills = on; args.changed = true; }
    },
    afterDatasetsDraw(chart) {
        ...
        chart.data.datasets.forEach((ds, di) => {
            ...
            meta.data.forEach((el, i) => {
                ...
                if (v == null || typeof v === 'object' || isNaN(+v) || !+v) return;
                let txt = null;
                if (cb) try { txt = cb({ chart, parsed: { x: +v, y: i }, raw: v, dataset: ds, dataIndex: i, datasetIndex: di }); } catch (e) { }
                ...
                if (ctx.roundRect) { ctx.beginPath(); ctx.roundRect(x, y - h / 2, w, h, 8); ctx.fill(); }
                else ctx.fillRect(x, y - h / 2, w, h);
                ctx.beginPath();
                ctx.arc(x + 8, y, 2.5, 0, Math.PI * 2);
                ctx.fillStyle = typeof col === 'string' ? col : '#5A8CAE';
                ctx.fill();
                ctx.fillStyle = '#FFF';
                ctx.fillText(txt, x + 14, y + .5);
            });
        });
        ctx.restore();
    }
...
function ttEl() {
    ...
    if (!el) {
        ...
        el.id = 'chartjs-tt';
        el.style.cssText = 'position:absolute;pointer-events:none;z-index:9999;background:#3B4046;color:#FFF;border-radius:12px;padding:10px 14px;font:11px/1.55 Verdana,sans-serif;box-shadow:0 8px 20px rgba(19,48,63,.30);opacity:0;transition:opacity .12s;white-space:nowrap';
        document.body.appendChild(el);
    }
    return el;
}

function ttPct(chart, dp) {
    if (!dp) return '';
    ...
    if (i < 1) return '';
    const raw = v => (v && typeof v === 'object') ? NaN : +v;
    const cur = raw(dp.dataset.data[i]), prev = raw(dp.dataset.data[i - 1]);
    if (!isFinite(cur) || !isFinite(prev) || !prev) return '';
    const p = (cur - prev) / Math.abs(prev) * 100, up = p >= 0;
    if (!p) return '';
    return ` <b style="color:${up ? '#7FE06C' : '#FF8A8C'}">${(up ? '+' : '') + p.toFixed(1).replace('.', ',')}% ${up ? '↑' : '↓'}</b>`;
}

function ttExternal(ctx) {
    ...
    const rows = (tooltip.body || []).map((b, i) => {
        ...
        let col = lc.backgroundColor;
        if (dp) {
            const mt = chart.getDatasetMeta(dp.datasetIndex).type;
            if (mt === 'line' && typeof dp.dataset.borderColor === 'string') col = dp.dataset.borderColor;
        }
        if (typeof col !== 'string') col = typeof lc.borderColor === 'string' ? lc.borderColor : '#5A8CAE';
        let txt = b.lines.join(' '), name = '', val = txt;
        const ci = txt.indexOf(':');
        if (ci > 0) { name = txt.slice(0, ci).trim(); val = txt.slice(ci + 1).trim(); }
        val = val.replace(/(-?(?:R\$\s?)?\d[\d.,]*\s?%?)/g, '<b>$1</b>');
        return `<div style="display:flex;align-items:center;gap:7px;margin-top:5px"><span style="width:8px;height:8px;border-radius:50%;background:${col};flex:0 0 8px"></span><span>${name ? `<b style="color:${colLight(col)}">${name}</b> : ` : ''}${val}${ttPct(chart, dp)}</span></div>`;
    }).join('');
...
const crosshair = {
    ...
    afterDatasetsDraw(chart) {
        ...
        ctx.setLineDash([4, 4]);
        ctx.lineWidth = 1;
        ctx.strokeStyle = 'rgba(0,56,101,.30)';
        ctx.moveTo(x, top);
        ctx.lineTo(x, bottom);
        ctx.stroke();
        ctx.restore();
...
const refLines = vals => ({
    ...
    afterDatasetsDraw(chart) {
        ...
        ctx.save();
        vals.forEach(v => {
            const x = xs.getPixelForValue(v.v);
...
function upgradeHeaders() {
    document.querySelectorAll('.panel h3').forEach(h => {
        if (h.dataset.up) return;
        h.dataset.up = '1';
        const link = h.querySelector('.link-tab');
        if (link) link.remove();
        ...
        if (h.children.length > 0) {
            ...
            if (TAGMAP[raw]) { t = TAGMAP[raw][0]; g = TAGMAP[raw][1]; }
                ...
                const m = /^(.+?)\s+(por|vs\.?)\s+(.+)$/.exec(raw);
                if (m) { t = m[1]; g = m[2] === 'por' ? cap(m[2] + ' ' + m[3]) : m[2] + ' ' + m[3]; }
            }
            titleHTML = t; tag = g;
        }
        h.innerHTML = '';
...
function solidCol(chart, di) {
    const ds = chart.data.datasets[di];
    if (chart.getDatasetMeta(di).type === 'line' && typeof ds.borderColor === 'string') return ds.borderColor;
    const b = ds.backgroundColor;
    return typeof b === 'string' ? b : (Array.isArray(b) ? b[0] : (typeof ds.borderColor === 'string' ? ds.borderColor : '#5A8CAE'));
}

function renderLegend(chart) {
    const p = chart.canvas.closest('.panel');
    if (!p) return;
    const el = p.querySelector('h3 .leg');
    if (!el) return;
    ...
    el.querySelectorAll('.lg-it').forEach(s => {
        s.onclick = () => {
