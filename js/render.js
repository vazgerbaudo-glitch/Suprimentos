...
function compRowHTML(cp,label,metricTxt,active,isGeral){return `<div class="comp-row${active?' active':''}${isGeral?' geral':''}" data-cp="${cp}"><div class="comp-avatar">${isGeral?'👥':initials(cp)}</div><div class="ci"><b>${label}</b><span>${metricTxt}</span></div></div>`;}
...
function renderCompIndividual(cp,team){
 ...
 const cwAll={},cwBuyers={};doneYAll.forEach(r=>{const w=isoWeek(r.dc);cwAll[w]=(cwAll[w]||0)+1;(cwBuyers[w]=cwBuyers[w]||new Set()).add(r.cp);});
