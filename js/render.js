function kpiCardHTML(k){return `<div class="kpi ${k.c||''}"><div class="lbl">${k.l}</div><div class="val">${k.v}</div>${k.p?`<span class="pill ${k.pc}">${k.p}</span>`:''}${k.n?`<div class="note">${k.n}</div>`:''}</div>`;}
function kpi(el,a){document.getElementById(el).innerHTML=a.map(kpiCardHTML).join('');}
function gaugeSVG(pct){
 ...
 const zones=[[0,80,'#BB4A4A'],[80,100,'#B59525'],[100,MAX,'#168980']];
 ...
 const np=pt(cl);
 return `<svg viewBox="0 0 180 95" style="width:100%;height:72px;overflow:visible">${arcs}<line x1="${cx}" y1="${cy}" x2="${np.x.toFixed(1)}" y2="${np.y.toFixed(1)}" stroke="#1C2325" stroke-width="3" stroke-linecap="round"/><circle cx="${cx}" cy="${cy}" r="5" fill="#1C2325"/></svg>`;
}
...
function renderProd(){
 ...
 const byW={};base.forEach(r=>{const w=isoWeek(r.dc);const o=(byW[w]=byW[w]||{ipd:0,ipc:0,bs:new Set(),cap:0,duM:new Map()});o.ipd+=r.ipd;o.ipc+=r.ipc;o.bs.add(r.cp);const capMeta=CAP[r.cl];if(capMeta>0)o.cap+=1/capMeta;if(r.du>0)o.duM.set(r.du,(o.duM.get(r.du)||0)+1);});
 const weeks=Object.keys(byW).sort(),ger=STATE.comp==='GERAL';
 let _fb=false;
 const wv=weeks.map(w=>{const o=byW[w];if(!ger)return o.ipd;if(o.ipc>0)return o.ipc;if(o.ipd>0&&o.bs.size){_fb=true;return o.ipd/o.bs.size;}return 0;});
 ...
 mkChart('c_gauge',{type:'bar',data:{labels:['Atingimento'],datasets:[{data:[Math.min(ating,200)],backgroundColor:ating>=100?C.teal:ating>=80?C.amber:C.red,borderRadius:6,barThickness:34}]},options:{indexAxis:'y',maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:()=>ating.toFixed(0)+'% da meta'}}},scales:{x:{min:0,max:200,grid:{color:'#EEF2F3'},border:{display:false},ticks:{callback:v=>v+'%'},afterBuildTicks:a=>{a.ticks=[{value:80},{value:100},{value:150}];}},y:noG}}});
 ...
 mkChart('c_psem',{type:'bar',data:{labels:cwk.map(w=>wkLabel(w)),datasets:[{data:cwk.map(w=>cw[w]),backgroundColor:cwk.map(w=>hl(w)?C.accent:'#BDCCD4'),borderRadius:3}]},options:{maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{...noG,ticks:{maxTicksLimit:13,font:{size:8}}},y:{...soG,beginAtZero:true}}}});
 ...
 const TCOL={Spot:'#3599B8',Urgente:'#BB4A4A',MRP:'#168980',Determinada:'#B59525',Contrato:'#9F4BB9','Regularização':'#5F6B6D',Outros:'#BDCCD4'};
 ...
 mkChart('c_esmeta',{type:'line',plugins:[crosshair],data:{labels:wksES.map(wkLabelFull),datasets:[{label:'Entrada',data:eV,borderColor:C.amber,backgroundColor:'rgba(181,149,37,.15)',fill:true,tension:.3,borderWidth:2,pointRadius:0,pointHoverRadius:5,pointHoverBackgroundColor:C.amber},{label:'Saída',data:sV,borderColor:C.teal,backgroundColor:'rgba(22,137,128,.18)',fill:true,tension:.3,borderWidth:2,pointRadius:0,pointHoverRadius:5,pointHoverBackgroundColor:C.teal},{label:'Meta',data:metaV,borderColor:C.red,borderDash:[6,4],borderWidth:1.6,pointRadius:0,pointHoverRadius:5,pointHoverBackgroundColor:C.red,tension:.2,fill:false}]},options:{maintainAspectRatio:false,interaction:{mode:'index',intersect:false},plugins:{legend:{position:'top',labels:{boxWidth:11,usePointStyle:true,font:{size:9}}},tooltip:{mode:'index',intersect:false,callbacks:{title:c=>'Semana de '+c[0].label}}},scales:{x:{...soG,ticks:{maxTicksLimit:13,font:{size:8},callback:function(v){return labES[v];}}},y:{...soG,beginAtZero:true}}}});
 ...
 const cellP=v=>{if(!v)return '<td class="cell" style="background:#EDF1F2;color:#BDCCD4">·</td>';const a=.14+.72*v/mxP;return `<td class="cell" style="background:rgba(53,153,184,${a.toFixed(2)});color:${a>.5?'#fff':'#003865'}">${v}</td>`;};
 document.getElementById('heat_prod').innerHTML=`<table><thead><tr><th class="rl"></th>${DOW.map(d=>`<th>${d}</th>`).join('')}<th>Total</th></tr></thead><tbody>${rowsP.map(r=>`<tr><td class="rl">${r[0]}</td>${r[1].map(cellP).join('')}<td class="cell" style="background:#1C2325;color:#fff">${r[2]}</td></tr>`).join('')||'<tr><td class="rl" colspan=7 style="color:#7F898A">Sem conclusões no recorte.</td></tr>'}</tbody></table>`;
...
function renderAging(){
 ...
 const FCOL=['#7FE06C','#4AC5BB','#FBD300','#F78344','#BB4A4A'];
 ...
 mkChart('c_afaixa',{type:'bar',data:{labels:FA.map(x=>x[0]),datasets:[{data:f,backgroundColor:FCOL,borderRadius:4}]},options:{maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:noG,y:{...soG,beginAtZero:true}}}});
 // Backlog por mês de criação (c_agbacklog)
 const bm={};ag.forEach(r=>{const k=ymKey(r.dl);bm[k]=(bm[k]||0)+1;});const bmk=Object.keys(bm).sort();
 mkChart('c_agbacklog',{type:'bar',data:{labels:bmk.map(mLabel),datasets:[{data:bmk.map(k=>bm[k]),backgroundColor:bmk.map((k,i)=>i<bmk.length-1?C.red:C.steel),borderRadius:4}]},options:{maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>c.parsed.y+' RCs abertas'}}},scales:{x:{...noG,ticks:{font:{size:9}}},y:{...soG,beginAtZero:true}}}});
 // Aging por comprador x faixa (c_agcompfaixa)
 const byCp={};ag.forEach(r=>{(byCp[r.cp]=byCp[r.cp]||[]).push(r.age);});
 ...
 const cellH=v=>{if(!v)return '<td class="cell" style="background:#EDF1F2;color:#BDCCD4">·</td>';const a=.14+.72*v/mxH;return `<td class="cell" style="background:rgba(187,74,74,${a.toFixed(2)});color:${a>.5?'#fff':'#6E2A2A'}">${v}</td>`;};
 document.getElementById('heat_aging').innerHTML=`<table><thead><tr><th class="rl"></th>${FA.map(fx=>`<th>${fx[0]}</th>`).join('')}<th>Total</th></tr></thead><tbody>${rowsH.map(r=>`<tr><td class="rl">${r[0]}</td>${r[1].map(cellH).join('')}<td class="cell" style="background:#1C2325;color:#fff">${r[2]}</td></tr>`).join('')||'<tr><td class="rl" colspan=7 style="color:#7F898A">Sem RCs abertas no recorte.</td></tr>'}</tbody></table>`;
 ...
 [0,.25,.5,.75,1].forEach(t=>{const x=L+plot*t;svg+=`<line x1="${x}" y1="20" x2="${x}" y2="${H-8}" stroke="#EEF2F3"/><text x="${x}" y="14" font-size="9" fill="#7F898A" text-anchor="middle">${Math.round(gmax*t)}d</text>`;});
 boxComps.forEach((s,i)=>{const a=s.ages,q1=quart(a,.25),md=quart(a,.5),q3=quart(a,.75),mn=a[0],mx=a[a.length-1],y=30+i*rowH,cy=y+7;
  svg+=`<text x="${L-6}" y="${cy+3}" font-size="10.5" fill="#293537" text-anchor="end">${s.cp}</text>`;
  svg+=`<line x1="${sx(mn)}" y1="${cy}" x2="${sx(mx)}" y2="${cy}" stroke="#7F898A"/><line x1="${sx(mn)}" y1="${cy-4}" x2="${sx(mn)}" y2="${cy+4}" stroke="#7F898A"/><line x1="${sx(mx)}" y1="${cy-4}" x2="${sx(mx)}" y2="${cy+4}" stroke="#7F898A"/>`;
  svg+=`<rect x="${sx(q1)}" y="${cy-7}" width="${Math.max(1,sx(q3)-sx(q1))}" height="14" fill="rgba(53,153,184,.18)" stroke="#3599B8"/><line x1="${sx(md)}" y1="${cy-7}" x2="${sx(md)}" y2="${cy+7}" stroke="#BB4A4A" stroke-width="2"/>`;});
 svg+='</svg>';
 document.getElementById('box_aging').innerHTML=boxComps.length?svg:'<div style="color:#7F898A;font-size:12px">Dados insuficientes para boxplot no recorte.</div>';
 ...
 mkChart('c_agevol',{type:'line',data:{labels:wk.map(wkLabel),datasets:[{label:'Ciclo médio',data:wk.map(w=>Math.round(byW[w].reduce((a,b)=>a+b,0)/byW[w].length)),borderColor:C.blue,backgroundColor:'rgba(53,153,184,.08)',fill:true,tension:.3,borderWidth:2,pointRadius:2}]},options:{maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>c.parsed.y+' dias de ciclo'}}},scales:{x:{...noG,ticks:{maxTicksLimit:10,font:{size:8}}},y:{...soG,beginAtZero:true}}}});
 // Evolução dos itens críticos >30d (c_agcrit)
 const critW={};concl.forEach(r=>{if(r.cyc>30)critW[r.w]=(critW[r.w]||0)+1;});
 mkChart('c_agcrit',{type:'line',data:{labels:wk.map(wkLabel),datasets:[{label:'Ciclo >30d',data:wk.map(w=>critW[w]||0),borderColor:C.red,backgroundColor:'rgba(187,74,74,.12)',fill:true,tension:.3,borderWidth:2,pointRadius:2}]},options:{maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{...noG,ticks:{maxTicksLimit:10,font:{size:8}}},y:{...soG,beginAtZero:true}}}});
 ...
 document.getElementById('funnel_aging').innerHTML=eta.map((x,i)=>{const w=Math.max(16,Math.round(x[1]/mxE*100));return `<div style="display:flex;align-items:center;gap:10px;margin:5px 0"><div title="${x[0]}" style="width:180px;flex:0 0 180px;font-size:11px;text-align:right;color:#5F6B6D;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${x[0]}</div><div style="height:26px;width:${w}%;background:hsl(${194-i*8},${55-i*2}%,${46-i}%);border-radius:4px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:11px;font-weight:700;min-width:32px">${x[1]}</div></div>`;}).join('')||'<div style="color:#7F898A;font-size:12px">Sem RCs abertas no recorte.</div>';
...
function renderSLA(){
 ...
 const atrasos=foraR.map(r=>r.sr-r.sa).filter(d=>d>0);const atrMed=atrasos.length?Math.round(atrasos.reduce((a,b)=>a+b,0)/atrasos.length):0;
 ...
 mkChart('c_slafaixa',{type:'bar',data:{labels:Object.keys(gf),datasets:[{data:Object.values(gf),backgroundColor:['#FBD300','#F78344','#BB4A4A','#7A2E2E'],borderRadius:4}]},options:{maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:noG,y:{...soG,beginAtZero:true}}}});
 ...
 const cell=v=>{if(!v)return `<td class="cell" style="background:#EDF1F2;color:#BDCCD4">·</td>`;const a=.14+.72*v/mx;return `<td class="cell" style="background:rgba(187,74,74,${a.toFixed(2)});color:${a>.5?'#fff':'#6E2A2A'}">${v}</td>`;};
 document.getElementById('heat').innerHTML=`<table><thead><tr><th class="rl"></th>${HF.map(f=>`<th>${f[0]} d</th>`).join('')}<th>Total</th></tr></thead><tbody>${rowsH.map(r=>`<tr><td class="rl">${r[0]}</td>${r[1].map(cell).join('')}<td class="cell" style="background:#1C2325;color:#fff">${r[2]}</td></tr>`).join('')||'<tr><td class="rl" colspan=6 style="color:#7F898A">Sem itens fora do SLA no recorte.</td></tr>'}</tbody></table>`;
 ...
 mkChart('c_mssla',{type:'bar',data:{labels:MS,datasets:[{data:msP,backgroundColor:msP.map(p=>p>=90?C.teal:p>=80?'#FBD300':p>=75?C.amber:C.red),borderRadius:4}]},options:{indexAxis:'y',maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>c.parsed.x+'% dentro do SLA'}}},scales:{x:{...soG,min:0,max:100,ticks:{callback:v=>v+'%'}},y:noG}}});
...
function renderSaving(){
 ...
 const bw={};base.forEach(r=>{if(!r.dc)return;const w=isoWeek(r.dc);bw[w]=(bw[w]||0)+(r.vp-r.vn);});const wk=Object.keys(bw).sort();
 mkChart('c_savsem',{type:'bar',data:{labels:wk.map(w=>wkLabel(w)),datasets:[{data:wk.map(w=>bw[w]),backgroundColor:C.teal,borderRadius:3}]},options:{maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>BRL(c.parsed.y)}}},scales:{x:{...noG,ticks:{maxTicksLimit:13,font:{size:8}}},y:{...soG,beginAtZero:true,ticks:{callback:Kf}}}}});
 // Saving por categoria (c_savcat)
 const bca={};base.forEach(r=>{const c=r.cat||'';if(!c)return;bca[c]=(bca[c]||0)+(r.vp-r.vn);});const cat=Object.entries(bca).sort((a,b)=>b[1]-a[1]).slice(0,10);
 mkChart('c_savcat',{type:'bar',data:{labels:cat.map(x=>x[0].slice(0,18)),datasets:[{data:cat.map(x=>x[1]),backgroundColor:C.steel,borderRadius:3}]},options:{indexAxis:'y',maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>BRL(c.parsed.x)}}},scales:{x:{...soG,ticks:{callback:Kf}},y:{...noG,ticks:{font:{size:9}}}}}});
 ...
 mkChart('c_savcomp',{type:'bar',data:{labels:co.map(x=>x[0]),datasets:[{data:co.map(x=>x[1]),backgroundColor:co.map(x=>x[0]===STATE.comp?C.accent:C.green),borderRadius:3}]},options:{indexAxis:'y',maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>BRL(c.parsed.x)}}},scales:{x:{...soG,ticks:{callback:Kf}},y:noG}}});
 ...
 const ttS=parS.reduce((a,x)=>a+x[1],0);let cumS=0,n80=0,hit80=false;
 ...
 document.querySelector('#t_saving tbody').innerHTML=detS.map(r=>`<tr><td>${r.rc||'-'}</td><td>${r.it||'-'}</td><td>${r.cp}</td><td>${(r.cat||'').trim()||'N/D'}</td><td class="num">${BRL(r.vp)}</td><td class="num">${BRL(r.vn)}</td><td class="num" style="${r.sv<0?'color:#BB4A4A':''}">${BRL(r.sv)}</td><td class="num" style="${r.sv<0?'color:#BB4A4A':''}">${r.vp?Math.round(r.sv/r.vp*100):0}%</td></tr>`).join('')||'<tr><td colspan=8 style="color:#7F898A">Sem itens com saving no recorte.</td></tr>';
 // Leitura (texto de insight)
...
function renderContr(){
 ...
 const OTH_PAL=[C.teal,C.amber,C.steel,C.red,C.green,'#FBD300','#4AC5BB','#B687AC','#BD7150','#7B4F71'];
 const colorMap={};typeList.forEach((t,i)=>{colorMap[t]=t==='Contrato'?CCON:t==='Spot'?C.blue:t==='N/D'?'#BDCCD4':OTH_PAL[(i-2)%OTH_PAL.length];});
 ...
 const cdCOL=k=>k==='G'?'#1E9F7F':k==='S'?'#3599B8':k==='R'?'#F78344':k==='N/D'?'#BDCCD4':'#9F4BB9';
 ...
 const byCat={};base.forEach(r=>{const c=catOf(r);const t=typeOf(r);const o=byCat[c]=byCat[c]||{};o[t]=(o[t]||0)+1;});
 ...
 const PAL=[CCON,C.blue,C.teal,C.amber,C.steel,C.red,C.green,'#7F898A'];
 ...
 mkChart('c_contrevol',{type:'line',data:{labels:wks.map(wkLabel),datasets:[
  {label:'Contrato',data:wks.map(w=>bw[w].Contrato),borderColor:CCON,backgroundColor:'rgba(159,75,185,.12)',fill:true,tension:.3,borderWidth:2,pointRadius:2},
  {label:'Spot',data:wks.map(w=>bw[w].Spot),borderColor:C.blue,backgroundColor:'rgba(53,153,184,.12)',fill:true,tension:.3,borderWidth:2,pointRadius:2},
  {label:'Outros tipos',data:wks.map(w=>bw[w].Outros),borderColor:'#7F898A',backgroundColor:'rgba(127,137,138,.10)',fill:true,tension:.3,borderWidth:2,pointRadius:2,borderDash:[4,3]}
 ...
 document.querySelector('#t_contr tbody').innerHTML=cats.map(x=>`<tr><td>${x.c}</td>${typeList.map(t=>`<td class="num">${x.o[t]||0}</td>`).join('')}<td class="num">${x.tot}</td><td class="num">${x.tot?Math.round((x.o['Contrato']||0)/x.tot*100):0}%</td></tr>`).join('')||`<tr><td colspan="${typeList.length+3}" style="color:#7F898A">Nenhuma RC liberada no recorte.</td></tr>`;
 // Leitura (texto de insight)
 const topCat=cats[0],nd=byCat['N/D'],ndTot=nd?Object.values(nd).reduce((a,v)=>a+v,0):0;
 const topOther=typesOther.find(t=>t!=='N/D');
 document.getElementById('ins-contr').innerHTML=base.length?`<b>Leitura:</b> no recorte entraram <b>${nCon} RCs de Contrato</b> e <b>${nSpo} RCs de Spot</b> (${pctCon.toFixed(0)}% / ${pctSpo.toFixed(0)}% do mix)${topOther?`, além de <b>${nOut} RCs em outros tipos</b> — o mais comum é <b>${topOther}</b> (${typeCounts[topOther]}). `:'. '}${topCat?`Carteira com maior volume: <b>${topCat.c}</b> (${topCat.tot} RCs). `:''}${ndTot?`<b style="color:#B59525">⚠ ${ndTot} RCs (${Math.round(ndTot/base.length*100)}%) estão sem Carteira/Categoria preenchida</b> — priorize o preenchimento para uma leitura confiável por carteira.`:''}`:'<b>Sem RCs liberadas no recorte.</b>';
...
function renderCompradores(){
 ...
 const rows=comps.map(cp=>{
  ...
  const byW={};done.forEach(r=>{const w=isoWeek(r.dc);byW[w]=(byW[w]||0)+r.ipd;});
 ...
 const ageColor=r=>r.agingAvg==null?'#7F898A':r.agingAvg<=15?C.teal:r.agingAvg<=30?C.amber:C.red;
 ...
 document.querySelector('#t_comp tbody').innerHTML=sorted.map(r=>{
  const slaTxt=r.slaPct!=null?r.slaPct.toFixed(1)+'%':'—',slaCol=r.slaPct==null?'':r.slaPct>=90?'#168980':r.slaPct>=80?'#B59525':'#BB4A4A';
  const ageTxt=r.agingAvg!=null?r.agingAvg+'d':'—',ageCol=r.agingAvg==null?'':r.agingAvg<=15?'#168980':r.agingAvg<=30?'#B59525':'#BB4A4A';
  const bg=r.cp===STATE.comp?'background:#FFFBE0':'';
  return `<tr class="jump" data-cp="${r.cp}" style="${bg}"><td>${r.cp}</td><td class="num">${r.concl.toLocaleString('pt-BR')}</td><td class="num">${r.ipd.toFixed(2)}</td><td class="num">${r.openN}</td><td class="num" style="color:${ageCol}">${ageTxt}</td><td class="num" style="color:${slaCol}">${slaTxt}</td><td class="num" style="${r.saving<0?'color:#BB4A4A':''}">${BRL(r.saving)}</td></tr>`;
 }).join('')||'<tr><td colspan="7" style="color:#7F898A">Nenhum comprador com dados no recorte.</td></tr>';
 ...
 document.querySelectorAll('#t_comp tbody tr.jump').forEach(tr=>tr.onclick=()=>{const cp=tr.dataset.cp;STATE.comp=STATE.comp===cp?'GERAL':cp;f_comp.value=STATE.comp;render();});
 // Leitura
...
function renderCompIndividual(cp,team){
 ...
 const cw={};doneY.forEach(r=>{const w=isoWeek(r.dc);cw[w]=(cw[w]||0)+1;});
 const cwk=Object.keys(cw).sort();
 // Itens/dia — média das semanas no recorte
 const byWI={};doneP.forEach(r=>{const w=isoWeek(r.dc);byWI[w]=(byWI[w]||0)+r.ipd;});
 ...
 const fCounts=FA.map(()=>0);agesP.forEach(a=>fCounts[faIdx(a)]++);
 ...
 const bwS={};slaY.forEach(r=>{const w=isoWeek(r.dc);(bwS[w]=bwS[w]||{i:0,t:0});bwS[w].t++;if(r.ss==='I')bwS[w].i++;});
 ...
 const bwV={};savP.forEach(r=>{if(!r.dc)return;const w=isoWeek(r.dc);bwV[w]=(bwV[w]||0)+(r.vp-r.vn);});
 ...
 mkChart('c_ind_prod',{type:'bar',data:{labels:cwk.map(wkLabel),datasets:[{data:cwk.map(w=>cw[w]),backgroundColor:C.accent,borderRadius:2}]},options:{maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{...noG,ticks:{maxTicksLimit:8,font:{size:8}}},y:{...soG,beginAtZero:true}}}});
 ...
 mkChart('c_ind_sla',{type:'line',data:{labels:wkS.map(wkLabel),datasets:[{data:wkS.map(w=>Math.round(bwS[w].i/bwS[w].t*100)),borderColor:C.blue,backgroundColor:'rgba(53,153,184,.1)',fill:true,tension:.3,borderWidth:2,pointRadius:0}]},options:{maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{...noG,ticks:{maxTicksLimit:8,font:{size:8}}},y:{...soG,min:0,max:100,ticks:{callback:v=>v+'%'}}}}});
...
function renderOverview(){
 ...
 kpi('kpi-overview',[
  {l:'Itens concluídos',v:P.concluidos.toLocaleString('pt-BR'),c:pCor,n:'atingimento '+P.ating.toFixed(0)+'% da meta'},
 ...
 mkChart('c_ov_contr',{type:'doughnut',data:{labels:['Contrato','Spot','Outros'],datasets:[{data:[K.nCon,K.nSpo,K.nOut],backgroundColor:['#9F4BB9',C.blue,'#BDCCD4'],borderWidth:0}]},options:{maintainAspectRatio:false,cutout:'62%',plugins:{legend:{position:'bottom',labels:{boxWidth:11,usePointStyle:true,font:{size:10}}}}}});
