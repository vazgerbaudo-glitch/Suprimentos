...
function renderProd(){
 ...
 const ctx=ALL.filter(r=>r.st==='C'&&r.dc>=DATA_INI&&inY(r.dc)&&compHit(r)&&tpHit(r));const cw={};ctx.forEach(r=>{const w=isoWeek(r.dc);cw[w]=(cw[w]||0)+1;});
 const ew={};ALL.filter(r=>r.dl&&r.dl>=DATA_INI&&inY(r.dl)&&compHit(r)&&tpHit(r)).forEach(r=>{const w=isoWeek(r.dl);ew[w]=(ew[w]||0)+1;});
 ...
 const byWY={};ctx.forEach(r=>{const w=isoWeek(r.dc);const o=(byWY[w]=byWY[w]||{ipd:0,ipc:0,bs:new Set()});o.ipd+=r.ipd;o.ipc+=r.ipc;o.bs.add(r.cp);});
